-- Check if booking_extensions table already exists, create it if not
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'booking_extensions') THEN
        ...
        -- Add new notification type for booking extensions if it doesn't already exist
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'notification_type') THEN
            CREATE TYPE notification_type AS ENUM ('BOOKING_CREATED', 'BOOKING_UPDATED', 'BOOKING_CANCELED', 'BOOKING_EXTENDED');
        ELSE
